...
class Sever {
    ...
    private setupMongooseConnection(): void {
        mongoose
            .connect(`mongodb://${config.database.hostname}:${config.database.port}/${config.database.hostname}`)
            .then((_) => {
                logging.info(this.name, `Connected to ${config.database.hostname}:${config.database.port}!`);
            })
            .catch((error) => {
    ...
    private setupServer(): void {
        ...
        this.server.listen(config.server.port, () => logging.info(this.name, `Setup Server on ${config.server.hostname}:${config.server.port}`));
    }
